...
export class AlgorandEncoder extends Encoder {
     ...
     * @returns
     */
    encodeSignedTransaction(stx) {
        const encodedTxn = new Uint8Array(msgpack.encode(stx, { sortKeys: true, ignoreUndefined: true }));
        return encodedTxn;
    }
    /**
     *
    ...
    encodeTransaction(tx) {
        ...
        const encoded = msgpack.encode(tx, { sortKeys: true, ignoreUndefined: true });
        // tag
        const TAG = Buffer.from("TX");
        // concat tag + encoded
        const encodedTx = Encoder.ConcatArrays(TAG, encoded);
        return encodedTx;
    }
    /**
     *
    ...
    decodeTransaction(encoded) {
        const TAG = Buffer.from("TX");
        const tagBytes = TAG.byteLength;
        // remove tag Bytes for the tag and decode the rest
        const decoded = msgpack.decode(encoded.slice(tagBytes));
        return decoded;
    }
    /**
     *
     ...
     * @returns
     */
    decodeSignedTransaction(encoded) {
